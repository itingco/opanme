...
$path = __DIR__.'/../../app/Services/BarcodeImportService.php';
$source = file_get_contents($path);

if ($source === false) {
    fwrite(STDERR, "FAIL: BarcodeImportService.php cannot be read\n");
    exit(1);
}

foreach ([
    'lookup array chunk' => 'array_chunk(array_keys($payloadByBarcode), self::LOOKUP_CHUNK_SIZE)',
    'upsert array chunk' => 'array_chunk(array_values($payloadByBarcode), self::UPSERT_CHUNK_SIZE)',
] as $label => $needle) {
...
// ItemBarcode upsert writes exactly ItemCode, Barcode and UOM = 3 bindings/row.
...
if (str_contains($source, "'created_at' => now()") || str_contains($source, "'updated_at' => now()")) {
    fwrite(STDERR, "FAIL: DB_AppHub ItemBarcode has no timestamp columns; payload should stay lean\n");
    exit(1);
}

echo "barcode import batching contract: PASS (lookup={$lookupChunk}, upsert={$upsertChunk}, max_upsert_bindings=".($upsertChunk * 3).")\n";
